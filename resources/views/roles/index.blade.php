<x-app-layout>
    ...
    <div>
        <form class="mb-3" action="{{ route('roles.store') }}" method="POST">
            @csrf
            ...
            @forelse($roles as $item)
                <tr>
                    ...
                    <td class="px-6 py-3 whitespace-nowrap text-sm text-center">
                        <a href="{{ route('roles.edit', $item->id) }}" class="text-blue-500">Edit</a> |
                        <form class="inline" action="{{ route('roles.destroy', $item->id) }}" method="POST">
                            @method('DELETE')
                            @csrf
